...
print([[
description:
	This script will attempt to read the current df-screen, and if is is a 
...
character encoding:
	...
	function format_for_forum(strin)
		local strout = strin
		...
		local line = ""
			...
			if lines ~= nil then
				...
				for n,x in ipairs(lines) do 
					if (x ~= nil) and (x[0] ~= nil) then 
						log:write(format_for_forum(x[0]), ' ')  
						--log:write(x[0],'\n')  
					end 
				end
				log:write("[/color]\n")
