-- Displays age, birth, maxage, shearing, milking, grazing, body size, and death info about a unit. Recommended keybinding Alt-I
-- version 1.02
...
local DEATH_TYPES = {
    ...
    ' killed by falling rocks',		-- FALLING_ROCKS
    ' experienced death by CHASM',	-- CHASM
    ' experienced death by CAGE',	-- CAGE
    ' was murdered',				-- MURDER
    ' experienced death by TRAP',	-- TRAP
    ' vanished',					-- VANISH
...
function UnitInfoViewer:init(args) -- requires args.unit
	...
	if df.unit:is_instance(args.unit) then
		...
		if not (self.ident.dead or self.ident.undead or self.ident.ghostly) then --not self.dead
			...
			if self.ident.unit.flags1.tame then
...
function UnitInfoViewer:chunk_BodySize()
	local i = self.ident
	local bs1 = i.caste.body_size_1 -- body size at body_size_2 days old
	local bs2 = i.caste.body_size_2 -- days of age to reach body_size_1
	local bam = i.caste.body_appearance_modifiers
	local ubam = i.unit.appearance.body_modifiers
	local age_d = i.age_time:getDays() -- float
	-- y = (rise/run)*x+init
	local rise, run, init_x, init_y = 0,0,0,0
	for idx,v in ipairs(bs2) do
	...
	-- if creature only has one size defined, no growth
	-- if unit has reached max growth, rise = 0
	local bs = (run > 0) and ( (rise/run) * (age_d - init_x) + init_y ) or init_y
	--[[
	for i,v in ipairs(ubam) do
		if bam[i].type >=0 and bam[i].type <=2 then -- HEIGHT BROADNESS LENGTH
			bs = bs * (v/100)
		end
	end
	--]]
