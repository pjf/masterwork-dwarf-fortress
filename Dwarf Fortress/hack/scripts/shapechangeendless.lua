    ...
    script.start(function()
        local ok = 
            script.showYesNoPrompt(
                ...
                " into a monster for ever?",
                COLOR_BROWN)
        if ok then dialog.showListPrompt("Creature Selection","Choose creature:",COLOR_WHITE,tbl,f) end
    end)
end
local args = {...}
selectCreature(tonumber(args[1]),tonumber(args[2]),tonumber(args[3]),{'DRAGON','HYDRA','GRIFFON_FF','BALROG','MANTICORE','SPIDER_BLADE_GIANT_QUEEN','OBSIDIAN_COLOSSUS','OGRE_DUSK','BEBILITH','FROST_WRAITH','BASILISK'})
