...
# if the 2nd parameter is 'obsidian', obsidian rain for the targets instead of instant death
obsidian = ($script_args[1] == 'obsidian')
...
slayit = lambda { |u|
	if not obsidian
		...
		# !!WARNING!! do not call on a obsidian-safe creature
		ouh = df.onupdate_register("slayrace ensure #{u.id}", 1) {
			if u.flags1.dead
				...
				df.map_tile_at(x, y, z).spawn_water(3)
				df.map_tile_at(x, y, z).spawn_magma(3)
			end
		}
	end
}
