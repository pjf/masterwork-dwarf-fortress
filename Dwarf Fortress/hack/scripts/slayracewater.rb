...
# if the 2nd parameter is 'water', water rain for the targets instead of instant death
water = ($script_args[1] == 'water')
...
slayit = lambda { |u|
	if not water
		...
		# !!WARNING!! do not call on a water-safe creature
		ouh = df.onupdate_register("slayrace ensure #{u.id}", 1) {
			if u.flags1.dead
				...
				df.map_tile_at(x, y, z).spawn_water(7)
			end
		}
	end
}
